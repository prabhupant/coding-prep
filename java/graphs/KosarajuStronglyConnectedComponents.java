...
public class KosarajuStronglyConnecetedComponents{

    public List<Set<Vertex<Integer>>> scc(Graph<Integer> graph){
        
        //to hold vertices by finish time in reverse order
        ...
        while(!stack.isEmpty()){
            ...
            //we have used stack.poll() above in place of stack.pop() because .poll() will return null value on empty conidtion whereas .pop() will throw an exception
            if(visited.contains(vertex)){
                continue;
            }
    ...
    private void DFSUtil(Vertex<Integer> vertex, Set<Vertex<Integer>> visited, Deque<Vertex<Integer>> stack){
        ...
        stack.offerFirst(vertex); //inserts the specified element at the front, if added, returns true, otherwise false
    }
